...
def simulate(conf, state, sp, wp, weather_dict, actor_list):
    ...
    autoware_stuck = 0
    goal_msg, proc_state, pub_cmd = None, None, None
    ...
    try:
        ...
        add_car_frame, blueprint_library, clock, player_bp, town_map, vehicle_bp_library = simulate_initialize(client,
                                                                                                               ...
                                                                                                               weather_dict,
                                                                                                               world)
        autoware_container, ego, goal_ow, goal_ox, goal_oy, goal_oz, player, max_steer_angle = ego_initialize(
            agents_now,
            blueprint_library, conf,
            ego, goal_loc, goal_rot,
            player_bp, sensors,
            sp, state, town_map,
            username, world, wp)
        if conf.agent_type == c.AUTOWARE:
            carla_error, goal_msg, proc_state, pub_cmd = autoware_launch(actor_vehicles, actor_walkers, carla_error,
                                                                         conf, goal_loc, goal_ow, goal_ox, goal_oy,
                                                                         goal_oz, state, world)
        ...
        try:
            ...
            while True:
                ...
                # # Carla's agents are running in synchronous mode,
                ...
                break_flag, retval = check_destination(actor_vehicles, actors_now, agents_now, autoware_stuck, conf,
                                                       goal_loc, goal_msg, player_loc, proc_state, pub_cmd, retval,
        ...
        save_video(autoware_container, carla_error, conf, username)
...
def check_destination(actor_vehicles, actors_now, agents_now, autoware_stuck, conf, goal_loc, goal_msg, player_loc,
                      proc_state, pub_cmd, retval, s_started, sensors, speed, state):
...
def save_video(autoware_container, carla_error, conf, username):
    if conf.agent_type == c.BEHAVIOR:
        # remove jpg files
...
def autoware_launch(actor_vehicles, actor_walkers, carla_error, conf, goal_loc, goal_ow, goal_ox, goal_oy, goal_oz,
                    state, world):
    ...
    world.tick()
    return carla_error, goal_msg, proc_state, pub_cmd


def ego_initialize(agents_now, blueprint_library, conf, ego, goal_loc, goal_rot, player_bp, sensors, sp, state,
                   town_map, username, world, wp):
    # for autoware
    autoware_container = None
    goal_ow, goal_ox, goal_oy, goal_oz = 0, 0, 0, 0
    player = None
    ...
    world.tick()  # sync with simulator
    return autoware_container, ego, goal_ow, goal_ox, goal_oy, goal_oz, player, max_steer_angle
